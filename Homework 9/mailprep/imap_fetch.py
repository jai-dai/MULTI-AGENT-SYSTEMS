...
def resolve_folders(imap: imaplib.IMAP4_SSL, wanted: list[str],
    ...
    `quiet` — для пробы при старте агента: там это служебное сообщение влезает
    в середину вопроса к человеку. Отсутствие папки сообщается всегда: это уже
    не справка, а расхождение конфига с сервером.
    """
...
def count_new(host: str, user: str, password: str, folders: list[str],
              db_path: str) -> dict:
    """Сколько новых писем ждёт на сервере. Тела НЕ качаются.

    Это `UID SEARCH` и ничего больше — секунды против минут у полной выкачки.
    Нужно, чтобы спрашивать «грузить N писем?» можно было дёшево и при каждом
    запуске: вопрос, который сам по себе стоит минуту ожидания, человек
    начнёт пропускать.

    Ошибка соединения — не повод не запускать агента: возвращаем 0 и причину.
    Работать с уже собранным индексом можно и без сети.
    """
    ...
    try:
        ...
        total, per_folder = 0, {}
        for folder in resolve_folders(imap, folders, quiet=True):
            ...
            known_validity, last_uid = store.sync_state(conn, folder)
            if known_validity is not None and known_validity != validity:
                # UIDVALIDITY сменилась — прежние UID недействительны, и сколько
                # там «нового», отсюда не видно. Честнее сказать «неизвестно»,
                # чем показать число, которое ничего не значит.
                return {"total": None, "folders": {},
                        "error": f"UIDVALIDITY изменилась в {folder}"}
            status, data = imap.uid("SEARCH", None, f"UID {last_uid + 1}:*")
            uids = data[0].split() if status == "OK" and data and data[0] else []
            # `UID n:*` всегда возвращает хотя бы одно письмо, даже когда нового нет.
            fresh = len([u for u in uids if int(u) > last_uid])
            if fresh:
                # Имя папки — человеку, а он не читает mUTF-7: Gmail отдаёт
                # «Отправленные» как `[Gmail]/&BB4EQgQ,BEAEMAQyBDsENQQ9BD0ESwQ1-`.
                per_folder[decode_mutf7(folder)] = fresh
            total += fresh
        return {"total": total, "folders": per_folder, "error": None}
