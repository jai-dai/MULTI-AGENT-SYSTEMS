...
# Что добавилось ради наблюдаемости

Промпт супервизора приезжает из Langfuse, как и у остальных. И `ainvoke` в
`main.py` получает колбэк, который заводит корневой span — под него встанут и
собственные LLM-вызовы супервизора, и всё, что придёт от суб-агентов через
проброшенный контекст.
...
class Supervisor:
    ...
    def __init__(self) -> None:
        ...
        self.agent = None
        # Контекст трассировки, снятый ТАМ, где он ещё есть — в `run_once`,
        # внутри собственного root span. Инструменты ниже читают его отсюда:
        # полагаться на окружение внутри `async def` нельзя (см. `a2a_utils`).
        self.trace_carrier: dict | None = None
    ...
    def _tools(self):
        ...
        async def delegate_to_planner(request: str) -> str:
            ...
            raw = await a2a_utils.ask(AGENT_URLS["planner"], request,
                                  carrier=supervisor.trace_carrier)
    ...
    async def build(self):
        ...
        self.agent = create_agent(
            ...
            system_prompt=observability.prompt("supervisor"),
