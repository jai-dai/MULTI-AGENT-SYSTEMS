...
def rerank(query: str, candidates: list[tuple[str, int]],
    ...
    # Здесь стоял ещё один порог — «весь пул на нуле, значит у реранкера нет
    # мнения, сохраняем порядок слияния». Он снят, и обе причины замерены.
    #
    # Держался он на одном наблюдении: запрос «чому постачальник затримав
    # відповідь по котлу» против английской переписки давал ВЕСЬ пул на
    # 0.0000-0.0001, потому что bge-reranker-base не сопоставляет языки между
    # собой. Это чинит `_query_for` переводом запроса под язык пассажа — тот же
    # пул после перевода упорядочен, нужное письмо первое. Порог лечил симптом
    # причины, которой больше нет.
    #
    # Подобрать его так и не вышло: по величине порога релевантности набор
    # терял по три попадания на каждой метрике (24/27 -> 21/27 по документам,
    # 15/22 -> 12/22 по пассажам), потому что 0.0099 против 0.0019 — оба ниже
    # порога, но это настоящий порядок. А низкое значение не срабатывало
    # никогда. Метка `WEAK` и так говорит пользователю то же самое, не трогая
    # порядок выдачи.
...
def retrieve(query: str, top_k: int = None, top_n: int = None,
    ...
    results = [{
        ...
        # Признак письма — `message_id`: он есть у тела письма и у вложения, и
        # ни у одного документа с диска. Вложение отличается от тела наличием
        # `date_source`, который проставляет только ingest.py, читая файл.
        #
        # Раньше проверялся один лишь `date_source`, и это ломалось на индексах,
        # собранных ДО того, как даты стали проставляться всем: учебный корпус
        # из трёх PDF целиком объявлялся перепиской, и агент цитировал
        # «письмо: retrieval-augmented-generation.pdf #1 в цепочке». Признак
        # обязан говорить о самом чанке, а не о версии конвейера, которым его
        # нарезали.
        "index": key[0],
        "kind": ("attachment" if chunk.get("message_id") and chunk.get("date_source")
                 else "mail" if chunk.get("message_id") else "document"),
