...
| `REQUEST_TIMEOUT` | ні | `60` | таймаут завантаження сторінки в `read_url` |
| `MODEL_TIMEOUT` | ні | `300` | таймаут одного виклику моделі |
| `TEMPERATURE` | ні | `0` | ігнорується для reasoning-моделей (`gpt-5*`, `o*`) |
...
## Зміна моделі: що ламається насправді

«Провайдер міняється одним рядком» — правда лише для коду. Перехід на `gpt-5.1` на
великому дослідницькому запиті одразу виявив три несумісності, і кожна валила запуск.

**1. `temperature=0` більше не приймається.** Reasoning-моделі (`gpt-5*`, `o*`) працюють
лише зі значенням за замовчуванням і відповідають 400 на будь-яке інше. Параметр тепер
надсилається вибірково:
...
**2. Один таймаут обслуговував дві різні речі.** `request_timeout = 60` йшов і в клієнт
OpenAI, і в `DOWNLOAD_TIMEOUT` для `trafilatura`. Для `gpt-4.1-mini` шістдесяти секунд
вистачало з запасом, а `gpt-5.1` думає над одним викликом довше — обидві репліки впали з
`APITimeoutError` після двох ретраїв. Просто збільшити число було не можна: тоді зависла
сторінка тримала б агента ті самі пʼять хвилин. Тому таймаути розведені:

```
REQUEST_TIMEOUT = 60    # завантаження сторінки — коротко, сайт не має стопорити агента
MODEL_TIMEOUT   = 300   # виклик моделі — reasoning-моделі легітимно думають хвилинами
```

**3. Частина сайтів блокує дефолтний user-agent.** `bittensor.com`,
`ritualfoundation.org` та інші віддавали 403 на UA бібліотеки `trafilatura`. Після
підстановки браузерного UA `bittensor.com` почав віддавати текст із першої спроби.

### Що показав прогон на сильнішій моделі

Той самий код, той самий промпт, ті самі інструменти — інша поведінка:

```
репліка 1 (огляд ландшафту):   5 кроків, 31 tool call, ~117k токенів
репліка 2 (порівняльний аналіз): 4 кроки,  3 tool call, ~200k токенів
```

- **Ширша паралельність.** До 10 `read_url` в одному кроці проти 2-3 у `gpt-4.1-mini`.
- **Модель сама обійшла проблему анотацій.** Замість `arxiv.org/abs/...` вона одразу
  запитувала `arxiv.org/html/...`, а для старої статті знайшла сторонній повнотекстовий
  рендерер `ar5iv.labs.arxiv.org` — тобто підказка `[ABSTRACT ONLY]` в описі інструмента
  спрацювала не буквально, а як стратегія.
- **Наочна ціна памʼяті.** Друга репліка зробила лише 3 виклики інструментів, але
