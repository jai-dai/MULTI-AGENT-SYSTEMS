...
class Settings(BaseSettings):
    ...
    planner_port: int = 8903
    researcher_port: int = 8904
    critic_port: int = 8905
...
#
# В версии со своим циклом ключ читался из настроек и передавался в клиент явно.
# LangChain так не умеет: `create_agent(model="gpt-5.2")` собирает клиента сам и
# берёт ключ из ПЕРЕМЕННОЙ ОКРУЖЕНИЯ. Наш `.env` читает pydantic-settings — он
# кладёт значения в объект `settings`, а не в `os.environ`, поэтому LangChain их
# не видит и падает с «Missing credentials» при полностью правильном .env.
#
# Экспорт стоит здесь, а не в каждой точке входа: серверов теперь пять, и
# забыть про один из них — вопрос времени. Существующее окружение не трогаем:
# переменная, выставленная снаружи, должна побеждать файл.
# --------------------------------------------------------------------------- #
...
# Промпты агентов
#
# Задание требует держать их здесь («config.py — Prompts + settings + ports»),
# и это единственное отличие раскладки от hw9, где под них заведён отдельный
# `prompts.py`. Тексты те же самые: меняется транспорт, а не роли.
# --------------------------------------------------------------------------- #
