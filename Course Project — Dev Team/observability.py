...
def handler(trace_ctx=None):
    ...
    # ВАЖНО: клиент должен быть создан ДО хендлера. `CallbackHandler` не берёт
    # ключи из настроек — он ищет ГЛОБАЛЬНО инициализированный клиент Langfuse,
    # а тот регистрируется при создании. Не создашь заранее — получишь
    # «Langfuse client initialized without public_key» и молча выключенную
    # трассировку при совершенно правильном .env.
    #
    # В предыдущей работе это работало СЛУЧАЙНО: там промпты приезжали из
    # Langfuse, и `get_prompt` инициализировал клиент раньше. Убери промпты — и
    # трассировка исчезла бы, а причина была бы не видна.
    client()
