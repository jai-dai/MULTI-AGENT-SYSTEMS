...
def main():
    ...
    # Почта добирается ДО того, как агент откроет индексы: локальный Qdrant
    # пускает к каталогу один процесс, поэтому запись и чтение здесь разведены
    # во времени, а не в инфраструктуре. Подробности — в mailsync.py.
    mailsync.offer_refresh()
