...
| Запит | OpenAI (1536) | e5-base локально (768) |
|---|---|---|
| EN, точний термін<br>«what is retrieval augmented generation» | RAG p.1, p.6, p.5 | RAG p.1, p.5, p.6 — **ті самі сторінки** |
| EN, перефразування<br>«how do I split long documents» | RAG p.4 — *«Chunking involves various strategies for breaking up the data into vectors»* | LLM p.4 (BPE-токенізація), langchain p.4 — **список літератури** |
| UA<br>«що таке доповнена пошуком генерація» | RAG p.6, p.3, p.4 | RAG **p.1**, p.6, p.5 |
| RU<br>«как разбивать документы на фрагменты» | RAG p.4 (сторінка про chunking) | langchain p.4 (бібліографія), LLM p.26 |

Висновки не однобічні:

- **На точному терміні — нічия**, обидві дають ті самі сторінки.
- **На українському запиті e5 виграє**: витягла `p.1` — сторінку з визначенням,
  тоді як OpenAI почала з `p.6`. Це те, заради чого беруть багатомовну модель.
- **На перефразуваннях e5 програє помітно.** Тексти перевірені, не вгадані:
  OpenAI повернула сторінку, де дослівно сказано *«Chunking involves various
  strategies…»*, а e5 пішла в токенізацію BPE і в **список літератури**.

Багатомовність не безкоштовна: 278M параметрів, розмазаних по сотні мов, проти
спеціалізованої моделі. Тому вибір залежить від корпусу — для українських і
російських документів e5 доречніша, для англійських OpenAI сильніша.
