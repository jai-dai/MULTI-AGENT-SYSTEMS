...
| Формат | Читається | Нюанс |
|---|---|---|
| `.pdf` | посторінково | сторінка потрапляє в цитату: `[договір.pdf p.7]` |
| `.docx` | абзаци **і таблиці** | у файлі немає поняття сторінки — усі чанки як `p.1` |
| `.txt` `.md` `.rst` | цілком | — |
| `.xlsx` `.pptx` | ні | поки що пропускаються мовчки |
