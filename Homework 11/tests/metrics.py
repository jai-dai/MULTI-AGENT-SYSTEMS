...
#
# Правило подъёма было «наблюдённый минимум минус 0.1», и второй прогон показал,
# что запас мал: на НЕИЗМЕНИВШЕМСЯ входе Answer Relevancy дал 0.723 против 0.893,
# то есть разброс судьи 0.17. Один поднятый порог пришлось вернуть (см. ниже).
#
# Отсюда честная формулировка правила: порог, выведенный из ОДНОГО наблюдения на
# пример, ненадёжен по построению. Поднимать стоит только там, где запас заведомо
# больше разброса, и до тех пор, пока разброс не замерен, — консервативно.
THRESHOLDS = {
    ...
    # ВОЗВРАЩЁН на 0.7 после второго прогона, и это самый полезный урок таблицы.
    # По первому замеру (сред 0.91, мин 0.85, n=6) порог был поднят до 0.8. На
    # втором прогоне ТОТ ЖЕ САМЫЙ, неизменившийся запрос получил 0.723 вместо
    # 0.893 — разброс 0.17 на идентичном входе. Правило «минимум минус 0.1»
    # оказалось уже разброса самого судьи.
    #
    # Вывод общий, а не про эту метрику: порог, выведенный из ОДНОГО наблюдения
    # на пример, не порог. Чтобы поднимать осознанно, нужен разброс — несколько
    # прогонов судьи на одном и том же входе. Такого замера здесь нет.
